...
class LeakyUNet(nn.Module):
    ...
    def down(in_channels, out_channels):
        return nn.Sequential(
            nn.MaxPool2d(2),
            LeakyUNet.double_conv(in_channels, out_channels)
        )
    ...
    def up(in_channels, out_channels, bilinear=True):
        if bilinear:
            ...
            up_layer = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)

        return nn.Sequential(
            up_layer,
            LeakyUNet.double_conv(in_channels, out_channels, in_channels // 2)
        )
